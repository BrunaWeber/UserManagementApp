        ...
        private void buttonLogin_Click(object sender, EventArgs e)
        {
            ...
            if (db.OpenConnection())
            {
                ...
                foreach (User user in userList)
                {
                    ...
                    if (this.textBoxUsername.Text == user.Email)
                    {
                        userFound = true;
                ...
                // Warn user if email (Username) is not found

                if (userFound == false)
                {
                    MessageBox.Show($"The user: {this.textBoxUsername.Text} does not exist. Please try again.\n\n(If problems persist, please contact the I.T. department.)", "User not found", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
